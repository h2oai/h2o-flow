...
(function() {
  ...
  gulp.task('build-scripts', function() {
    return gulp.src(['src/**/*.coffee']).pipe(ignore.exclude(/tests.coffee$/)).pipe(coffee({
      bare: false
    })).pipe(concat('flow.js')).pipe(expand('desugar.yml')).pipe(header('"use strict";(function(){ var lodash = window._; window.Flow={}; window.H2O={};')).pipe(footer('}).call(this);')).pipe(gulp.dest(config.dir.deploy + '/js/'));
  });

  gulp.task('build-tests', function() {
    return gulp.src(['src/**/*.coffee']).pipe(coffee({
      bare: false
    })).pipe(concat('flow-tests.js')).pipe(expand('desugar.yml')).pipe(header('"use strict";(function(){ var lodash = require(\'lodash\'); var test = require(\'tape\'); var Flow={}; var H2O={};')).pipe(footer('}).call(this);')).pipe(gulp.dest('./build/js/'));
  });

  gulp.task('build-headless-test', function() {
    return gulp.src(['test/**/*.coffee']).pipe(coffee({
      bare: true
    })).pipe(concat('headless-test.js')).pipe(gulp.dest('./build/js/'));
  });
  ...
  gulp.task('build-styles', function() {
    return gulp.src(['src/*.styl']).pipe(stylus({
      use: [nib()]
